...
class Trail extends THREE.Group {
  ...
  constructor(props) {
    ...
    const geometry = props.geometry || new MeshLineGeometry();
    const material = props.material || new MeshLineMaterial({
      lineWidth: 0.1,
      sizeAttenuation: 1,
  ...
  rebuildTrail(props) {
    ...
    if (td.points.length !== pointsArrayLength) {
      td.points = Float32Array.from({
        length: pointsArrayLength
      }, (_, i) => tmp.worldPosition.getComponent(i % 3));
    }
  ...
  updateLineMesh() {
    ...
    const geometry = this.trailData.geometry;
    geometry.setPoints(points, attenuation);
  }
  updateInstances() {
    const instanceMesh = this.trailData.mesh;

...
function getBounds(primitiveBounds, offset, count, target2, centroidTarget) {
  let minx = Infinity;
  let miny = Infinity;
  let minz = Infinity;
  let maxx = -Infinity;
...
function generateIndirectBuffer(ranges, useSharedArrayBuffer) {
  ...
  const length2 = ranges.reduce((acc, val) => acc + val.count, 0);
  const byteCount = useUint32 ? 4 : 2;
  const buffer = useSharedArrayBuffer ? new SharedArrayBuffer(length2 * byteCount) : new ArrayBuffer(length2 * byteCount);
...
ExtendedTriangle.prototype.intersectsTriangle = function() {
  ...
  function coplanarIntersectsTriangle(self, other, target2, suppressLog) {
  ...
  function intersectTriangleSegment(triangle4, degenerateTriangle, target2, suppressLog) {
    ...
    if (isNearZero(startDist)) {
      if (isNearZero(endDist)) {
        return coplanarIntersectsTriangle(triangle4, degenerateTriangle, target2, suppressLog);
      } else {
        if (target2) {
          target2.start.copy(segment.start);
          target2.end.copy(segment.start);
        }
        return triangle4.containsPoint(segment.start);
      }
    } else if (isNearZero(endDist)) {
      if (target2) {
        target2.start.copy(segment.end);
        target2.end.copy(segment.end);
      }
      return triangle4.containsPoint(segment.end);
    } else {
      if (triangle4.plane.intersectLine(segment, tmpVec) != null) {
        if (target2) {
          target2.start.copy(tmpVec);
          target2.end.copy(tmpVec);
        }
  ...
  function intersectTrianglePoint(triangle4, degenerateTriangle, target2) {
    const point = degenerateTriangle.a;
    if (isNearZero(triangle4.plane.distanceToPoint(point)) && triangle4.containsPoint(point)) {
      if (target2) {
        target2.start.copy(point);
        target2.end.copy(point);
      }
      return true;
    } else {
      return false;
  ...
  function intersectSegmentPoint(segmentTri, pointTri, target2) {
  ...
  function handleDegenerateCases(self, other, target2, suppressLog) {
    if (self.isDegenerateIntoSegment) {
      if (other.isDegenerateIntoSegment) {
        ...
        if (isNearZero(z1 - z2)) {
          if (target2) {
            target2.start.copy(segment1.start).addScaledVector(delta1, t);
            target2.end.copy(segment1.start).addScaledVector(delta1, t);
          }
          return true;
        } else {
          return false;
        ...
        return intersectSegmentPoint(self, other, target2);
      } else {
        return intersectTriangleSegment(other, self, target2, suppressLog);
      }
    } else if (self.isDegenerateIntoPoint) {
      if (other.isDegenerateIntoPoint) {
        if (other.a.distanceToSquared(self.a) < ZERO_EPSILON_SQR) {
          if (target2) {
            target2.start.copy(self.a);
            target2.end.copy(self.a);
          }
          return true;
        } else {
          return false;
        ...
        return intersectSegmentPoint(other, self, target2);
      } else {
        return intersectTrianglePoint(other, self, target2);
      }
    } else {
      if (other.isDegenerateIntoPoint) {
        return intersectTrianglePoint(self, other, target2);
      } else if (other.isDegenerateIntoSegment) {
        return intersectTriangleSegment(self, other, target2, suppressLog);
      }
    }
  }
  return function intersectsTriangle(other, target2 = null, suppressLog = false) {
    ...
    const res = handleDegenerateCases(this, other, target2, suppressLog);
    if (res !== void 0) {
      return res;
    }
    ...
    if (findIntersectionLineBounds(this, a1Proj, b1Proj, c1Proj, a1b1Dist, a1c1Dist, a1Dist, b1Dist, c1Dist, bounds1, edge1)) {
      return coplanarIntersectsTriangle(this, other, target2, suppressLog);
    }
...
var VertexAttributeTexture = class extends DataTexture {
  ...
  updateFrom(attr) {
    ...
    const length2 = finalStride * dimension * dimension;
    const dataArray = new targetBufferCons(length2);
    ...
    for (let g = 0, l2 = geometries.length; g < l2; g++) {
      ...
      if (!skip) {
        if (key === "color" && targetAttribute.itemSize !== attr.itemSize) {
          for (let index = offset, l3 = attr.count; index < l3; index++) {
            attr.setXYZW(index, targetAttribute.getX(index), targetAttribute.getY(index), targetAttribute.getZ(index), 1);
          }
        } else {
...
function boneNormalTransform(mesh, index, target2) {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    if (weight !== 0) {
      ...
      addScaledMatrix(_matrix3, _boneMatrix2, weight);
    }
  }
  _matrix3.multiply(mesh.bindMatrix).premultiply(mesh.bindMatrixInverse);
  target2.transformDirection(_matrix3);
  return target2;
}
...
var PathTracingSceneGenerator = class {
  ...
  generate(onProgress = null) {
    ...
    if (!needsMaterialIndexUpdate) {
      for (let i = 0, length2 = materials.length; i < length2; i++) {
        const material = materials[i];
...
var FloatAttributeTextureArray = class extends DataArrayTexture {
  ...
  updateAttribute(index, attr) {
    ...
    const length2 = width * height * 4;
    const offset = length2 * index;
...
var MaterialsTexture = class extends DataTexture {
  ...
  updateFrom(materials, textures) {
    ...
    for (let i = 0, l = materials.length; i < l; i++) {
      ...
      index += writeTextureMatrixToArray(m2, "alphaMap", floatArray, index);
    }
    const hash = bufferToHash(floatArray.buffer);
    if (this.hash !== hash) {
      this.hash = hash;
...
var GradientEquirectTexture = class extends ProceduralEquirectTexture {
  constructor(resolution = 512) {
    ...
    this.generationCallback = (polar, uv, coord, color) => {
      _direction2.setFromSpherical(polar);
      const t = _direction2.y * 0.5 + 0.5;
